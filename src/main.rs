...
/// iterations it took for `c` to leave the cricle of radius two centered on
...
/// `pixel` is a (row, column) pair indicating a particular pixel in that
...
/// Write the buffer `pixels`, whose dimensions are given by the `bounds`, to
